...
float dequeue(struct Queue* queue) { // Eliminar un elemento de la cola
	if (queue->front == NULL) {
		return 0; // Valor por defecto si la cola est� vac�a
	}
...
// Funci�n para contar cu�ntas l�neas tiene el archivo de texto
...
// Funci�n para crear un nuevo nodo y agregarlo al final de la lista
...
// Funci�n para calcular consumo total acumulado (Joaquin Contreras)
...
// Funci�n para calcular el consumo de un mes seleccionado por el usuario (Facundo Degani)
...
// Funci�n para identificar el dispositivo menos eficiente (Martin Gonzalez)
...
// Funci�n para calcular costo por mes (Valentin Albertinazzi)
...
float calcularPromedioConsumoDiario(struct Node* lista, int mesSeleccionado, int diaSeleccionado) {
	...
	if (count == 0) {
		return 0; // No hay datos para el d�a y mes especificados
	}
...
void tendenciaConsumo(struct Node* lista, int mesSeleccionado, int diaSeleccionado)
{
	...
	if (count == 0) {
		...
		return; // No hay datos para el día y mes especificados
	}
...
// Funci�n para identificar dispositivos que superan un umbral de consumo en un d�a y mes (FACUNDO DEGANI)
void identificar_Dispositivos_Umbral(struct Node *lista,int dia,int mes,float umbral){
	
	printf("Dispositivos que superan el umbral de consumo de %.2f kWh en el d�a %d del mes %d:\n",umbral,dia,mes);
	...
	// Funci�n para calcular el costo por tipo de dispositivo (ALBERTINAZZI VALENTIN)
	...
	void encontrarDispositivoMasEficiente(struct Stack* stack) 
	{
		...
		if (maxEficiencia > 0) 
		{
			printf("El dispositivo más eficiente es:\n");
...
int main() {
	...
	do {
		...
		printf("\n8.identificar los dispositivos que superan un cierto umbral de consumo en un d�a y mes.");
		...
		case 6: {
			...
			printf("Ingresa el mes y el d�a para calcular el promedio de consumo diario (MM DD):");
			...
			if (promedioDiario==0){
				printf("No hay datos para el d�a y mes especificados.\n");
			} else {
